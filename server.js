...
app.get('/api/export/employees', requireAdmin, (req, res) => {
  res.setHeader('Content-Type', 'application/json; charset=utf-8');
  res.setHeader('Content-Disposition', 'attachment; filename="employees.json"');
  res.send(JSON.stringify(readJson(EMPLOYEES_FILE, []), null, 2));
});

app.get('/api/export/logs', requireAdmin, (req, res) => {
  res.setHeader('Content-Type', 'application/json; charset=utf-8');
  res.setHeader('Content-Disposition', 'attachment; filename="logs.json"');
  res.send(JSON.stringify(readJson(LOGS_FILE, []), null, 2));
});

app.get('/api/export/sites', requireAdmin, (req, res) => {
  res.setHeader('Content-Type', 'application/json; charset=utf-8');
  res.setHeader('Content-Disposition', 'attachment; filename="sites.json"');
  res.send(JSON.stringify(readJson(SITES_FILE, []), null, 2));
});

app.get('/api/export/all-data', requireAdmin, (req, res) => {
  const payload = {
  ...
  res.setHeader('Content-Type', 'application/json; charset=utf-8');
  res.setHeader('Content-Disposition', 'attachment; filename="clockflow-backup.json"');
